...
function fix_long_url( $url, $unsafe_url ) {
	...
	// Remove any trailing spaces from the long URL (edited by MGI)
	$url = preg_replace( '/(%20)+$/', '', $url );
	
	// MGI Addition : remove any spaces between the automatically entered http:// and the user-entered url
	$protocol = yourls_get_protocol( $url );
	$regexProtocol = str_replace( '/', '\/', $protocol );
	$url = preg_replace( '/^'.$regexProtocol.'(%20)+/', $protocol, $url );
